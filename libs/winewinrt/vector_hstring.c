/* WinRT Vector<HSTRING> implementation
 *
...
static HRESULT WINAPI iterable_view_hstring_First( IIterable_HSTRING *iface, IIterator_HSTRING **value )
{
    ...
    if (!(iter = calloc( 1, sizeof(struct iterator_hstring) ))) return E_OUTOFMEMORY;
...
static HRESULT WINAPI vector_hstring_GetView( IVector_HSTRING *iface, IVectorView_HSTRING **value )
{
    ...
    {
        hr = WindowsDuplicateString( impl->elements[i], &view->elements[view->size++] );
...
{
    struct vector_hstring *impl = impl_from_IVector_HSTRING( iface );
    HSTRING tmp, *elements = impl->elements;
    HRESULT hr;
...
HRESULT vector_hstring_copy( IIterable_HSTRING *iterable, IVector_HSTRING **out )
{
    struct vector_hstring *impl;
