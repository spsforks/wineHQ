...
/* derived from libsystemd, under LGPL 2.1 license
 * https://github.com/systemd/systemd/blob/main/src/basic/hmac.c
 */
#define HMAC_BLOCK_SIZE    64
#define INNER_PADDING_BYTE 0x36
#define OUTER_PADDING_BYTE 0x5c

void hmac_sha256(const void *key, size_t key_size, const void *input, size_t input_size, uint8_t res[SHA256_DIGEST_SIZE])
{
    ...
    {
        sha256_init(&hash);
        sha256_update(&hash, input, input_size);
        sha256_finalize(&hash, replacement_key);
    ...
    sha256_init(&hash);
    sha256_update(&hash, inner_padding, HMAC_BLOCK_SIZE);
    sha256_update(&hash, input, input_size);
    sha256_finalize(&hash, res);

    /* Second pass: hash the outer padding array and the result of the first pass. */
    sha256_init(&hash);
    sha256_update(&hash, outer_padding, HMAC_BLOCK_SIZE);
    sha256_update(&hash, res, SHA256_DIGEST_SIZE);
    sha256_finalize(&hash, res);
}
