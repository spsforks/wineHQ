...
struct voice_information_vector
{
    ...
    struct synth_voices_provider provider;
};
...
static HRESULT WINAPI factory_ActivateInstance( IActivationFactory *iface, IInspectable **instance )
{
    ...
    ISpeechSynthesizerOptions_AddRef(&impl->options->ISpeechSynthesizerOptions_iface);
...
static HRESULT dummy_provider_init(struct synth_voices_provider *provider)
{
    ...
    if (FAILED(hr))
    {
        free(provider->voices);
    }
    else
        provider->num_voices = 1;
    return hr;
}
...
static HRESULT WINAPI installed_voices_static_get_AllVoices( IInstalledVoicesStatic *iface, IVectorView_VoiceInformation **value )
{
    ...
    if (all_voices.provider.num_voices == 0)
        hr = dummy_provider_init(&all_voices.provider);
    else
        hr = S_OK;
    if (SUCCEEDED(hr))
