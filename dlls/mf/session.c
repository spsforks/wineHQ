...
static void transform_stream_push_sample(struct transform_stream *stream, IMFSample *sample)
{
    PROPVARIANT value = {.vt = VT_UNKNOWN, .punkVal = (IUnknown *)sample};
    IMFMediaEvent *event;

    if (SUCCEEDED(MFCreateMediaEvent(MEMediaSample, &GUID_NULL, S_OK, &value, &event)))
    {
        IMFCollection_AddElement(stream->events, (IUnknown *)event);
        IMFMediaEvent_Release(event);
    }
}
...
static void transform_stream_push_format_change(struct transform_stream *stream, IMFMediaType *media_type)
{
    ...
    if (SUCCEEDED(MFCreateMediaEvent(MEStreamFormatChanged, &GUID_NULL, S_OK, &value, &event)))
    {
        IMFCollection_AddElement(stream->events, (IUnknown *)event);
        IMFMediaEvent_Release(event);
    }
...
static void transform_stream_push_events(struct transform_stream *stream, IMFCollection *events)
{
    IMFMediaEvent *event;
...
{
    HRESULT hr = IMFCollection_RemoveElement(stream->events, 0, (IUnknown **)event);
    return hr == E_INVALIDARG ? MF_E_TRANSFORM_NEED_MORE_INPUT : hr;
}
    ...
    switch (node->type)
    {
        ...
        case MF_TOPOLOGY_TRANSFORM_NODE:
            for (i = 0; i < node->u.transform.input_count; ++i)
            {
                ...
                IMFCollection_Release(node->u.transform.inputs[i].events);
            }
            for (i = 0; i < node->u.transform.output_count; ++i)
            {
                transform_stream_drop_events(&node->u.transform.outputs[i]);
                IMFCollection_Release(node->u.transform.outputs[i].events);
            }
...
static HRESULT session_set_transform_stream_info(struct topo_node *node)
{
    ...
    if (SUCCEEDED(hr))
    {
        ...
        for (i = 0; i < input_count; ++i)
            MFCreateCollection(&streams[i].events);
        ...
        if (SUCCEEDED(hr = IMFTransform_GetOutputCurrentType(node->object.transform, id, &old_output_type)))
        {
                    ...
                    &new_output_type)))
            {
                if (buffers[i].pEvents || SUCCEEDED(hr = MFCreateCollection(&buffers[i].pEvents)))
                    transform_stream_push_format_change(stream, new_output_type);
                IMFMediaType_Release(new_output_type);
            }
...
static HRESULT transform_node_pull_samples(const struct media_session *session, struct topo_node *node)
{
    ...
    for (i = 0; i < node->u.transform.output_count; ++i)
    {
        ...
        if (buffers[i].pEvents)
            transform_stream_push_events(stream, buffers[i].pEvents);
        ...
        if (SUCCEEDED(hr))
        {
            ...
            transform_stream_push_sample(stream, buffers[i].pSample);
        }
    }
        ...
        UINT input, IMFSample *sample)
{
    ...
    if (sample)
    {
        ...
        if (hr == MF_E_NOTACCEPTING)
            transform_stream_push_sample(stream, sample);
    }
    else
    {
        ...
        UINT input, IMFMediaType *media_type)
{
    ...
    if (!support_dynamic_format_change)
    {
        ...
        {
            transform_stream_push_format_change(stream, media_type);
            return hr;
        }
    }
